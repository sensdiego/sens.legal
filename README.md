...
|---------|------------------|-----|------|
| Valter | Stage 9 of 10 - Scale + public presence<br>`[#########-]` | Ship similar cases expansion to production. Scale entity resolution cross-tribunal with legal provenance. Plan AWS infrastructure migration (NuageIT). OAuth 2.0 for App Directory. v0.6.0 live, 77K nodes/128K edges, 5.3K decisions in graph, scale-up target 800K. | Scale to 800K+ decisions in the pipeline. AWS deployment via NuageIT. Multi-tenancy and SLA guarantees. Cross-tribunal divergence detection. |
| Juca | Stage 3 of 5 - Polish and expand<br>`[######----]` | Polish judge profile catalog UX. Replace temporary production auth bypass. Expand catalog-driven intelligence products. Briefing progressivo shipped, migration closed. | Multi-agent orchestration (Valter, Leci). Cost ledger and session security. PWA. |
